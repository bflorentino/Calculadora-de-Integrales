...
namespace Selenium
{
    public class Webdriver
    {
        public static string MathSolver(string ecuacion, string lInferior, string lSuperior, string variable)
        {
            ...
            options.AddArguments("headless","disable-infobars", "--no-sandbox", "--disable-dev-shm-usage",
            ...
            using (IWebDriver driver = new ChromeDriver(driverService, options))
            {
                try
                {
                    ...
                    IWebElement vari = wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementExists(By.Id("int-var")));
